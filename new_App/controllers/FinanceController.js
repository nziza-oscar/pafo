...
const Implementation = require("../models/f_implementation");
...
exports.impProjectList = async(req,res)=>{
        ...
        include:[
            {model: fComponent,
              include:[
                 ...
                 {model: fActivity, include:[{model: fActivity, as:'Subactivities'}]}
              ]
            }
        ]
    });
    // return res.json(projects)
    res.render("im_ProjectList",{title:"ADD COMPONENTS",data: projects})
}

exports.impComponentList = async (req, res) => {
    ...
    try {
        ...
        // return res.json(components)
         ...
         res.render("imp_ComponentList",{title:`${req.project.name}`,project:req.project ,components:components,projectId:req.project.id})
    } catch (error) {
        console.error("Error fetching components:", error);
        res.status(500).json({ message: "Internal server error compnents", error:error.message });
    }
};



exports.impComponent = async (req, res) => {
     const {budgetId} = req.params   
    try {
         const budget = await BudgetPlanning.findByPk(budgetId,{
            include:[
        ...
        // return res.json(budget);              
         ...
         res.render("imp_component",{title:`Component Implementation`, budget})
    } catch (error) {
        console.error("Error fetching components:", error);
        res.status(500).json({ message: "Internal server error compnents", error:error.message });
    }
...
exports.impList = async (req, res) => {
    try {
      const implementations = await Implementation.findAll({
        include: ['BudgetPlannings']
      });
  
      res.render("imp_component", {
  ...
  exports.createImplementation = async (req, res) => {
    try {
      const { task, amount, doneBy, date, description, budgetPlanningId , year} = req.body;
  ...
  exports.editImplementationView = async (req, res) => {
    try {
      const implementation = await Implementation.findByPk(req.params.id);
  
      res.render("edit_implementation", {
        title: "Edit Implementation",
        implementation
      });
    } catch (error) {
      res.status(404).json({ message: "Implementation not found" });
    }
  };
  ...
  exports.updateImplementation = async (req, res) => {
    try {
      const { task, amount, doneBy, date, description, budgetPlanningId } = req.body;
  ...
  exports.deleteImplementation = async (req, res) => {
    try {
      ...
      res.status(500).json({ message: "Error deleting implementation", error: error.message });
    }
  };
